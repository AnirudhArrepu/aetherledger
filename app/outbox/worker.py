...
async def deliver_event(session, event):
    # placeholder: event.payload should include target URL and body
...
async def outbox_worker(poll_interval: float = 1.0):
    ...
    while True:
        async with Session() as s:
            q = await s.execute(
                "SELECT id, tx_id, payload, attempts, max_attempts FROM outbox_events WHERE delivered = false AND (next_retry_at IS NULL OR next_retry_at <= now()) ORDER BY created_at LIMIT 10 FOR UPDATE SKIP LOCKED"
            )
            rows = q.fetchall()
            for r in rows:
                ...
                if ok:
                    await s.execute("UPDATE outbox_events SET delivered = true WHERE id = :id", {"id": event.id})
                else:
                    #exp backoff
                    attempts = event.attempts + 1
                    if attempts >= event.max_attempts:
                        await s.execute("UPDATE outbox_events SET attempts = :a, next_retry_at = NULL WHERE id = :id", {"a": attempts, "id": event.id})
                    else:
                        delay = min(60, 2 ** attempts)
                        next_retry = datetime.now(timezone.utc) + timedelta(seconds=delay)
                        await s.execute("UPDATE outbox_events SET attempts = :a, next_retry_at = :n WHERE id = :id", {"a": attempts, "n": next_retry, "id": event.id})
            await s.commit()
